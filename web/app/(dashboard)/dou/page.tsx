...
export default async function DouPage({ searchParams }: { searchParams: Promise<SearchParams> }) {
  ...
  const { rows, total, page, pages } = await getAtos(filters)
    ...
    <div className="p-4 md:p-8 space-y-6">
      ...
      <div className="rounded-xl p-4 space-y-3" style={{ background: "var(--surface)", border: "1px solid var(--border)" }}>
        <form method="get" className="flex gap-2">
          <input name="q" defaultValue={filters.q} placeholder="Buscar por órgão ou título..."
          ...
          {filters.impacto && <input type="hidden" name="impacto" value={filters.impacto} />}
          {filters.secao && <input type="hidden" name="secao" value={filters.secao} />}
          {filters.data && <input type="hidden" name="data" value={filters.data} />}
      ...
      {pages > 1 && (
        <div className="flex justify-center gap-2">
          {page > 1 && (
            <Link href={`/dou?${new URLSearchParams({ ...(filters as Record<string, string>), page: String(page - 1) })}`}
