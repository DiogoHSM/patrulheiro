...
export default async function SenadoresPage({ searchParams }: { searchParams: Promise<{ sort?: string }> }) {
  const { sort } = await searchParams
  const [senadores, alinhamentoVotos] = await Promise.all([getSenadores(), getAlinhamentoVotos()])

  const sorted = [...senadores].sort((a, b) => {
  ...
  const partidos = [...new Set(senadores.map(s => s.partido).filter(Boolean))]
    .map(partido => {
      ...
      const totalClass = membros.reduce((acc, s) => acc + Number(s.total) - Number(s.pendentes), 0)
      const pctAlinhamento = totalClass > 0 ? Math.round((totalFav / totalClass) * 100) : 0
      const contrarias = membros.reduce((acc, s) => acc + Number(s.contrarias), 0)
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        {partidos.map(({ partido, membros, pctAlinhamento, contrarias }) => (
          <div key={partido} className="rounded-xl p-4" style={{ background: "var(--surface)", border: "1px solid var(--border)" }}>
            ...
            <p className="text-xs" style={{ color: "var(--text-muted)" }}>{membros.length} senador{membros.length !== 1 ? "es" : ""}</p>
            {pctAlinhamento > 0 && (
